...
def _run_daily_tasks(client, alpaca_client) -> None:
    ...
    try:
        ...
        raw_result = AIC.daily_market_analysis_v3(
            client,
...
def _run_daily_market_analysis_once() -> None:
    ...
    raw = AIC.daily_market_analysis_v3(client, market=market)
...
def _run_daily_market_analysis_v1_once() -> None:
    _setup_logging()
    market = input("Choose market (e.g. US, SWE, POL): ").strip() or "US"
    client = _build_gemini_client()
...
def _run_single_stock_analysis_once(stock_query: str) -> None:
    _setup_logging()
    client = _build_gemini_client()
    print(AIC.single_stock_analysis_v3(stock_query, client))


def _run_single_stock_analysis_v1_once(stock_query: str) -> None:
    _setup_logging()
    client = _build_gemini_client()
...
def _print_usage() -> None:
    print("Usage:")
    print("  python main.py                              # institutional 6-step market scan (v3)")
    print("  python main.py daily-market-analysis        # v3: institutional 6-step scan (default)")
    print("  python main.py daily-market-analysis-v2     # v2: 1-12 month scan, grade A/B only")
    print("  python main.py daily-market-analysis-v1     # v1: original 30-180 day swing scan")
    print("  python main.py analyze-stock AMD            # v3: institutional single-asset analysis")
    print("  python main.py analyze-asset gold           # v3: institutional single-asset analysis")
    print("  python main.py analyze-stock-v2 AMD         # v2: 1-12 month swing analysis")
    print("  python main.py analyze-stock-v1 AMD         # v1: original swing analysis")
    print("  python main.py AMD                          # shorthand (v3)")
...
def _dispatch_cli(argv: list[str]) -> None:
    ...
    if command == "daily-market-analysis-v1":
        _run_daily_market_analysis_v1_once()
        return
    ...
    if command == "analyze-stock-v1":
        stock_query = " ".join(argv[1:]).strip()
        if not stock_query:
            raise SystemExit(
                "Usage: python main.py analyze-stock-v1 <ticker-or-asset-query>"
            )
        _run_single_stock_analysis_v1_once(stock_query)
        return
